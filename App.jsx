...
import { 
  ...
  ChevronDown, Archive, Sparkles, PartyPopper, Heart, ArrowLeft, Star, Building2
} from 'lucide-react';
...
// --- CONSTANTS ---
const DEPARTMENTS = ['Front Desk', 'Housekeeping', 'Food & Beverage', 'Maintenance', 'Concierge', 'Spa & Wellness', 'Valet/Parking', 'Security/General'];

const COMPLAINT_ACTIONS = [
  'apologized and resolved immediately', 'provided a complimentary F&B voucher', 'upgraded / changed your guest room',
  'ensured the maintenance repair was completed', 'provided a housekeeping recovery / amenity', 'adjusted / waived the charges on your bill',
  'escalated the matter to the Department Head', 'escalated the matter directly to the General Manager', 'logged the issue for immediate internal review'
];

const COMPLIMENT_ACTIONS = [
  'Thanked guest & logged on guest profile', 'Shared praise at daily briefing', 'Recognized staff member directly',
  'Added to monthly recognition board', 'Sent follow-up appreciation note'
];

const CORE_VALUES = [
  'The Authentic O-Smile (Proudly Diverse) 🌍', 'Going the Extra Mile (Drive) 🚀', 'The Ultimate Team Player (Respect) 🤝',
  'Brilliant Problem Solver (Audacity) 💡', 'Always Having My Back (Trust) 🛡️'
];
...
const SOP_FRAMEWORK = {
  quick: { label: 'Quick Resolve', color: 'bg-[#595733]/10 border-[#595733]/20 text-[#595733]', badge: 'text-[#595733] bg-[#595733]/10 border-[#595733]/20', authority: 'Line Staff Authorized', icon: <UserCheck size={18} className="text-[#595733]" />, steps: 'Empowered for immediate resolution at the point of service. Authorized to apply minor bill corrections, room adjustments, or courtesy food & beverage vouchers on the spot.' },
  intermediate: { label: 'Intermediate Escalation', color: 'bg-[#cf6231]/10 border-[#cf6231]/20 text-[#cf6231]', badge: 'text-[#cf6231] bg-[#cf6231]/10 border-[#cf6231]/20', authority: 'Supervisor / HOD Required', icon: <Users size={18} className="text-[#cf6231]" />, steps: 'Exceeds standard staff limits. The HOD or Duty Supervisor must take direct ownership, contact the guest, and implement structured service recovery within 15 to 45 minutes.' },
  critical: { label: 'Critical Intervention', color: 'bg-[#8e2a2a]/10 border-[#8e2a2a]/20 text-[#8e2a2a]', badge: 'text-[#8e2a2a] bg-[#8e2a2a]/10 border-[#8e2a2a]/20', authority: 'General Manager Mandate', icon: <ShieldAlert size={18} className="text-[#8e2a2a] animate-pulse" />, steps: 'Severe operational incident or high-profile threat. Requires immediate background escalation alerts sent to the GM. Executive leadership must step in physically to resolve.' }
};
...
export default function App() {
  ...
  if (currentView === 'feedback') return <FeedbackApp onBackToPortal={() => setCurrentView('portal')} />;
  if (currentView === 'recognition') return <RecognitionApp onBackToPortal={() => setCurrentView('portal')} />;
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#f6ebda] font-sans relative overflow-hidden items-center justify-center p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl animate-in fade-in zoom-in-95 duration-500 delay-150">
        <div onClick={() => setCurrentView('feedback')} className="bg-white border-2 border-[#003040] rounded-3xl p-8 md:p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-[#003040] hover:text-white transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-2 group">
          ...
          <p className="text-sm font-medium opacity-70 group-hover:opacity-90 max-w-[250px]">Log complaints, incidents, and praises. Trigger escalations and track daily operations.</p>
        </div>

        <div onClick={() => setCurrentView('recognition')} className="bg-gradient-to-br from-[#ffb131] to-[#f18a00] border-2 border-[#f18a00] rounded-3xl p-8 md:p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:from-[#f18a00] hover:to-[#cf6231] transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-2 text-white group">
          ...
          <p className="text-sm font-medium opacity-90 max-w-[250px]">Recognize your colleagues, celebrate the O-Smile, and crown the Employee of the Month!</p>
        </div>
      </div>
      <div className="absolute bottom-8 opacity-40 flex items-center text-xs font-bold text-[#003040] tracking-widest uppercase"><ShieldCheck size={14} className="mr-1.5" /> Secure Live Production Environment</div>
    </div>
  );
}
...
function RecognitionApp({ onBackToPortal }) {
  ...
  const submitCheer = async (e) => {
    ...
    try {
      ...
      await addDoc(entriesRef, { ...form, date: new Date().toISOString(), userId: user.uid });
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#fffdf9] font-sans relative overflow-hidden">
      {toast && <div className="absolute top-20 left-1/2 transform -translate-x-1/2 bg-[#f18a00] text-white px-6 py-4 rounded-full shadow-2xl z-50 text-sm font-bold flex items-center space-x-2 animate-bounce"><Sparkles size={20} /><span>{toast}</span></div>}
      <header className="bg-gradient-to-r from-[#ffb131] to-[#f18a00] text-white px-4 md:px-8 py-4 shadow-lg z-20 w-full flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <button onClick={onBackToPortal} className="bg-white/20 p-2 rounded-full hover:bg-white/40 transition-colors"><ArrowLeft size={20} className="text-white" /></button>
          <div><h1 className="text-xl md:text-2xl font-extrabold tracking-tight flex items-center">Team Cheers <Sparkles size={20} className="ml-2" /></h1><span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-white/80">Recognize Greatness</span></div>
        </div>
      </header>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {activeTab === 'give' && (
            <form onSubmit={submitCheer} className="bg-white p-6 md:p-8 rounded-3xl border border-orange-100 shadow-xl space-y-6 animate-in zoom-in-95 duration-300">
              <div className="text-center mb-6"><Heart size={48} className="text-[#cf6231] mx-auto mb-2" /><h2 className="text-2xl font-extrabold text-gray-800">Who made you smile today?</h2></div>
              <div><label className="block text-sm font-bold text-gray-700 mb-2">Colleague's Name</label><input required value={form.recipient} onChange={e=>setForm({...form, recipient: e.target.value})} className="w-full border-2 border-orange-100 rounded-2xl p-4 text-lg font-bold outline-none focus:border-[#f18a00] bg-orange-50/50 transition-colors" placeholder="Who are you cheering for?" /></div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Reason for Cheer (Core Value)</label>
                <div className="relative">
                  <select value={form.coreValue} onChange={e=>setForm({...form, coreValue: e.target.value})} className="w-full border-2 border-orange-100 rounded-2xl p-4 text-sm md:text-lg font-bold outline-none focus:border-[#f18a00] bg-orange-50/50 transition-colors appearance-none cursor-pointer text-[#cf6231]">{CORE_VALUES.map(v => <option key={v}>{v}</option>)}</select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-orange-400 pointer-events-none" size={24} />
                </div>
              </div>
              <div><label className="block text-sm font-bold text-gray-700 mb-2">Leave a nice message</label><textarea required value={form.message} onChange={e=>setForm({...form, message: e.target.value})} rows="3" className="w-full border-2 border-orange-100 rounded-2xl p-4 text-base outline-none focus:border-[#f18a00] bg-orange-50/50 transition-colors resize-none" placeholder="Thank you for always..." /></div>
              <div><label className="block text-sm font-bold text-gray-700 mb-2">From (Your Name)</label><input required value={form.sender} onChange={e=>setForm({...form, sender: e.target.value})} className="w-full border-2 border-orange-100 rounded-2xl p-4 text-base outline-none focus:border-[#f18a00] bg-white transition-colors" placeholder="Your name" /></div>
              <button type="submit" className="w-full bg-gradient-to-r from-[#ffb131] to-[#f18a00] text-white font-black text-xl py-5 rounded-2xl hover:scale-[1.02] transition-transform shadow-lg flex items-center justify-center"><PartyPopper size={24} className="mr-3" /> Send Cheer!</button>
            </form>
          )}
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-xl space-y-4">
                ...
                {leaderboard.map((member, i) => (
                  <div key={member.name} className="flex items-center justify-between p-4 bg-orange-50/50 rounded-2xl border border-orange-100/50">
                    ...
                    <span className="bg-white text-[#f18a00] font-black px-4 py-2 rounded-xl shadow-sm border border-orange-100 flex items-center">{member.points} <Sparkles size={14} className="ml-1.5" /></span>
                  </div>
                ))}
                {leaderboard.length === 0 && <div className="text-center py-10 text-gray-400 font-bold">No cheers yet this month. Be the first!</div>}
              </div>
            </div>
          )}
        </div>
...
function FeedbackApp({ onBackToPortal }) {
  ...
  useEffect(() => { fetch('https://open.er-api.com/v6/latest/USD').then(res => res.json()).then(data => { if (data && data.rates) setExchangeRates(data.rates); }).catch(err => console.error(err)); }, []);
  useEffect(() => { const int = setInterval(() => setTimeTicker(Date.now()), 30000); return () => clearInterval(int); }, []);
  useEffect(() => { onAuthStateChanged(auth, setUser); }, []);
  useEffect(() => {
    if (!user) return; 
    ...
    const unsubscribe = onSnapshot(entriesRef, (snapshot) => {
        ...
        setEntries(loadedEntries); setLastSync(new Date().toLocaleTimeString()); setLoading(false); setCloudError(null);
      }, (error) => { setCloudError(`Database Error: ${error.message}`); setLoading(false); }
    );
    return () => unsubscribe();
  }, [user]);
  ...
  const addEntry = async (newEntry, openWhatsAppTrigger) => {
    ...
    try {
      ...
      setActiveTab('history'); setHistoryFilter('all'); showToast('Entry saved securely to cloud!');
  ...
  const addComment = async (id, commentText, authorName) => {
    ...
    try {
      const entryRef = doc(db, 'artifacts', SHARED_APP_ID, 'public', 'data', 'feedback_entries', id);
      await updateDoc(entryRef, { comments: arrayUnion({ text: commentText, author: authorName || 'Staff', time: new Date().toISOString() }) });
      showToast('Team note successfully saved!');
    } catch (error) { showToast(`Failed to save note: ${error.message}`); }
  };
  ...
  const archiveResolvedTickets = async () => {
    ...
    } catch (error) { showToast(`Archive failed: ${error.message}`); }
  };

  if (loading && !cloudError) return <div className="flex flex-col h-screen w-full bg-[#f6ebda] items-center justify-center text-center font-sans"><Loader2 className="animate-spin text-[#003040] mb-4" size={48} /><p className="text-[#003040] font-medium animate-pulse">Syncing with secure cloud...</p></div>;

  return (
    <div className="flex flex-col h-screen w-full bg-[#f6ebda] font-sans relative overflow-hidden">
      {toast && <div className="absolute top-20 md:top-6 left-1/2 transform -translate-x-1/2 w-[90%] md:w-auto md:min-w-[300px] bg-gray-800 text-white px-6 py-4 rounded-xl shadow-2xl z-50 text-sm text-center font-medium animate-in fade-in slide-in-from-top-4 flex items-center justify-center space-x-3"><CheckCircle size={20} className="text-green-400 shrink-0" /><span>{toast}</span></div>}
      <header className="bg-[#003040] text-white px-4 md:px-6 py-4 shadow-md z-20 w-full flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <button onClick={onBackToPortal} className="bg-white/10 p-2 rounded-full hover:bg-white/20 transition-colors" title="Back to Portal"><ArrowLeft size={18} className="text-white" /></button>
          <div><h1 className="text-lg md:text-xl font-bold tracking-wide">Guest Tracker</h1><div className="flex items-center text-[9px] md:text-[10px] text-[#a0c8d2] font-semibold tracking-wider mt-0.5"><RefreshCw size={8} className="mr-1.5 animate-spin" /> Live Sync Active</div></div>
        </div>
        <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="bg-[#003040]/80 text-white border border-[#a0c8d2]/20 rounded p-1.5 text-xs outline-none focus:ring-2 focus:ring-[#a0c8d2] cursor-pointer font-semibold shadow-sm hover:bg-[#003040]"><option value="$">USD ($)</option><option value="€">EUR (€)</option><option value="£">GBP (£)</option><option value="R">ZAR (R)</option></select>
      </header>
      {cloudError && <div className="bg-[#8e2a2a] text-white px-4 py-3 text-xs md:text-sm font-semibold flex items-center justify-center text-center z-20 shadow-md"><AlertCircle size={16} className="mr-2 shrink-0" /> {cloudError}</div>}
          ...
          <nav className="flex-1 px-4 py-8 space-y-3">
            {['dashboard', 'add', 'history'].map((tab) => (
              <button key={tab} onClick={() => { setActiveTab(tab); if(tab==='history') setHistoryFilter('all'); }} className={`flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 ${activeTab === tab ? 'text-[#f18a00] bg-[#f18a00]/10 font-bold shadow-sm' : 'text-gray-500 hover:text-[#003040] hover:bg-gray-50 font-medium'}`}>
                {tab === 'dashboard' ? <Activity size={20} className="mr-3 shrink-0" /> : tab === 'add' ? <PlusCircle size={20} className="mr-3 shrink-0" /> : <List size={20} className="mr-3 shrink-0" />}<span className="text-sm capitalize">{tab === 'add' ? 'New Entry' : tab}</span>
              </button>
            ))}
          </nav>
        </aside>
      ...
      <nav className="md:hidden bg-white border-t border-gray-200 absolute bottom-0 w-full flex justify-around p-2 z-30 pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
        {['dashboard', 'add', 'history'].map((tab) => (
          <button key={tab} onClick={() => { setActiveTab(tab); if(tab==='history') setHistoryFilter('all'); }} className={`flex flex-col items-center p-2 rounded-lg w-1/3 transition-colors ${activeTab === tab ? 'text-[#f18a00] bg-[#f18a00]/10' : 'text-gray-500 hover:text-[#003040]'}`}>
            {tab === 'dashboard' ? <Activity size={24} /> : tab === 'add' ? <PlusCircle size={24} /> : <List size={24} />}<span className="text-xs mt-1 font-semibold capitalize">{tab === 'add' ? 'New' : tab}</span>
          </button>
        ))}
      </nav>
    </div>
...
function FeedbackDashboard({ entries, currency, exchangeRates, onOpenTicketsClick, onStatClick, onArchiveMonth }) {
  ...
  const filtered = useMemo(() => {
    return entries.filter(e => {
      ...
      if (range === 'month') return eDate.getMonth() === new Date().getMonth() && eDate.getFullYear() === new Date().getFullYear();
  ...
  const stats = useMemo(() => {
    ...
    const currentViewMultiplier = exchangeRates[CURRENCY_MAP[currency] || 'USD'] || 1;
    filtered.forEach(e => {
      if (e.type === 'compliment') comps++;
      else if (e.type === 'incident') { incidents++; cost += ((Number(e.cost) || 0) * currentViewMultiplier); if (e.status === 'open') open++; } 
      else { complaints++; cost += ((Number(e.cost) || 0) * currentViewMultiplier); if (e.status === 'open') open++; }
    });
  ...
  const generateEODReport = () => {
    const todaysEntries = entries.filter(e => new Date(e.date).toDateString() === new Date().toDateString());
    let detailsText = "";
    todaysEntries.forEach((e, i) => detailsText += `${i + 1}. [${e.type.toUpperCase()}] ${e.department} - ${e.guestName}\n   Issue/Reason: ${e.reason}${(e.type==='complaint'||e.type==='incident')?` | Status: ${e.status.toUpperCase()}`:''}\n\n`);
    window.location.href = `mailto:${HOD_EMAIL_LIST}?subject=${encodeURIComponent(`Daily Hotel Feedback Report - ${new Date().toLocaleDateString()}`)}&body=${encodeURIComponent(`HOTEL FEEDBACK EOD REPORT\nDate: ${new Date().toLocaleDateString()}\n\n--- DETAILED LOGS ---\n${detailsText || "No feedback logged today."}\n\nGenerated by Onomo Feedback Tracker`)}`;
  };
...
function FeedbackAddForm({ onSave, currency, exchangeRates }) {
  ...
  const [form, setForm] = useState({ guestName: '', guestEmail: '', guestPhone: '', department: DEPARTMENTS[0], reason: '', handledBy: '', actionTaken: COMPLAINT_ACTIONS[0], cost: '', status: 'resolved', followUpEmail: '', followUpPhone: '', guestEmailSent: false, managerEmailSent: false, escalationSent: false, staffMentioned: '' });
  const activeActionOptions = type === 'compliment' ? COMPLIMENT_ACTIONS : COMPLAINT_ACTIONS;

  useEffect(() => { if (!activeActionOptions.includes(form.actionTaken)) setForm(prev => ({ ...prev, actionTaken: activeActionOptions[0] })); }, [type, activeActionOptions, form.actionTaken]);

  const inferredSeverity = useMemo(() => { return (type === 'complaint' || type === 'incident') ? determineSOPSeverity(form.reason) : 'quick'; }, [form.reason, type]);

  const submit = (e) => {
    e.preventDefault();
    const normalizedCostInUSD = (Number(form.cost) || 0) / (exchangeRates[CURRENCY_MAP[currency] || 'USD'] || 1);
    const finalizedEntry = { ...form, type, date: new Date().toISOString(), sentiment: analyzeSentiment(form.reason), cost: normalizedCostInUSD, severity: inferredSeverity, isArchived: false, comments: [] };
      ...
      <div className="bg-white p-5 md:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <div className="md:col-span-2">
            ...
            <div className="relative">
              <select value={form.department} onChange={e=>setForm({...form, department: e.target.value})} className="w-full border border-gray-300 rounded-xl p-3 md:p-4 pr-10 text-sm outline-none bg-white focus:ring-2 focus:ring-[#003040] cursor-pointer appearance-none">{DEPARTMENTS.map(d => <option key={d}>{d}</option>)}</select>
