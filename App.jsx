...
export default function App() {
  ...
  if (currentView === 'recognition') {
    return <RecognitionApp user={globalUser} onBackToPortal={() => setCurrentView('portal')} />;
  }
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {leaderboard.length > 0 && (
                <div className="bg-gradient-to-b from-[#ffb131] to-[#f18a00] rounded-3xl p-8 text-center text-white shadow-2xl relative overflow-hidden">
                  ...
                  <div className="inline-block bg-white/20 px-6 py-2 rounded-full font-bold text-lg backdrop-blur-sm">
                    {leaderboard[0].points} Recognition Points
                  </div>
                </div>
              )}
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-xl space-y-4">
                <h3 className="font-bold text-xl text-gray-800 border-b border-gray-100 pb-3 flex items-center">
                   <Activity className="mr-2 text-[#cf6231]" size={20} /> Unified Ranking (Staff + Guests)
                </h3>
    ...
    <div className="flex flex-col h-screen w-full bg-[#f6ebda] font-sans relative overflow-hidden">
      ...
      <header className="bg-[#003040] text-white px-4 md:px-8 py-4 shadow-md z-20 w-full flex justify-between items-center">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-wide">Guest Feedback Tracker</h1>
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-6">
        <div onClick={() => onStatClick('compliment')} className="text-[#595733] bg-[#595733]/10 border-[#595733]/20 hover:bg-[#595733]/20 cursor-pointer transition-colors p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center min-h-[100px] active:scale-95">
          <span className="text-3xl md:text-4xl font-bold leading-none mb-2">{stats.comps}</span>
          <span className="text-xs md:text-sm font-semibold text-center opacity-80 uppercase tracking-widest break-words">Praises</span>
        </div>
        <div onClick={() => onStatClick('complaint')} className="text-[#8e2a2a] bg-[#8e2a2a]/10 border-[#8e2a2a]/20 hover:bg-[#8e2a2a]/20 cursor-pointer transition-colors p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center min-h-[100px] active:scale-95">
          <span className="text-3xl md:text-4xl font-bold leading-none mb-2">{stats.complaints}</span>
          <span className="text-xs md:text-sm font-semibold text-center opacity-80 uppercase tracking-widest break-words">Complaints</span>
        </div>
        <div onClick={() => onStatClick('incident')} className="text-[#003040] bg-[#a0c8d2]/30 border-[#a0c8d2]/40 hover:bg-[#a0c8d2]/50 cursor-pointer transition-colors p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center min-h-[100px] active:scale-95">
          <span className="text-3xl md:text-4xl font-bold leading-none mb-2">{stats.incidents}</span>
          <span className="text-xs md:text-sm font-semibold text-center opacity-80 uppercase tracking-widest break-words">Incidents</span>
        </div>
      </div>
      
      <div className="grid grid-cols-2 gap-4 md:gap-6">
        <div onClick={onOpenTicketsClick} className="bg-white border-red-200 text-[#8e2a2a] p-4 md:p-6 rounded-2xl border shadow-sm transition-all min-h-[100px] active:scale-[0.98] cursor-pointer hover:shadow-md hover:border-gray-300 flex flex-col justify-between">
          <div className="flex flex-col md:flex-row md:items-center items-start md:space-x-2 space-y-1 md:space-y-0 mb-3">
            <AlertCircle size={18} className="text-[#8e2a2a]" />
            <span className="text-xs md:text-sm font-bold opacity-70 uppercase tracking-widest">Tickets Open</span>
          </div>
          <span className="text-2xl md:text-3xl font-bold truncate block text-[#003040]">{stats.open}</span>
        </div>
        <div className="bg-white border-gray-200 text-[#003040] p-4 md:p-6 rounded-2xl border shadow-sm transition-all min-h-[100px] flex flex-col justify-between">
          <div className="flex flex-col md:flex-row md:items-center items-start md:space-x-2 space-y-1 md:space-y-0 mb-3">
            <Coins size={18} className="text-[#a0c8d2]" />
            <span className="text-xs md:text-sm font-bold opacity-70 uppercase tracking-widest">Resolution Cost</span>
          </div>
          <span className="text-2xl md:text-3xl font-bold truncate block text-[#003040]">{currency}{stats.cost.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="bg-white p-5 md:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <div className="md:col-span-2">
            ...
            <div className="relative">
              <select value={form.department} onChange={e=>setForm({...form, department: e.target.value})} className="w-full border border-gray-300 rounded-xl p-3 md:p-4 pr-10 md:pr-12 text-sm outline-none bg-white focus:ring-2 focus:ring-[#003040] cursor-pointer appearance-none">{DEPARTMENTS.map(d => <option key={d}>{d}</option>)}</select>
...
function FeedbackHistory({ entries, onResolve, onAddComment, onMarkEmailSent, currency, exchangeRates, filter, setFilter, ticker }) {
  ...
  const handleWhatsAppEscalation = (entry) => {
    ...
    const whatsappUrl = cleanedPhone ? `https://api.whatsapp.com/send?phone=${cleanedPhone}&text=${encodeURIComponent(msg)}` : `https://api.whatsapp.com/send?text=${encodeURIComponent(msg)}`;
                ...
                <div>
                  ...
                  {sla.isBreached && !entry.isArchived && (
                    <div className="mb-4 -mx-5 md:-mx-6 -mt-5 md:-mt-6 bg-[#8e2a2a] text-white font-bold uppercase text-[10px] md:text-xs tracking-wider p-2.5 flex items-center justify-center space-x-2 animate-pulse rounded-t-xl shadow-sm">
                      <Clock size={14} /><span>⚠️ SLA BREACHED: Overdue {sla.hours}h {sla.minutes}m</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center space-x-2 flex-wrap gap-y-2">
                      ...
                      {(entry.type === 'complaint' || entry.type === 'incident') && (
                        <span className={`text-[10px] md:text-xs px-2.5 py-1 rounded-full font-bold border whitespace-nowrap ${SOP_FRAMEWORK[activeSeverity].badge}`}>{SOP_FRAMEWORK[activeSeverity].label}</span>
                      )}
                  ...
                  <div className="bg-[#f6ebda]/30 rounded-xl p-3 border border-[#a0c8d2]/30">
                    ...
                    <div className="space-y-2 mb-3 max-h-24 overflow-y-auto pr-1">
                      {entry.comments?.map((c, i) => (
                        <div key={i} className="bg-white p-2 rounded-lg border border-gray-200 text-xs shadow-sm"><span className="font-bold text-[#cf6231]">{c.author}:</span> <span className="text-gray-700 break-words whitespace-pre-wrap">{c.text}</span></div>
                      ))}
                    </div>
                    ...
                    {sla.isBreached && !entry.isArchived && (
                      <div className="p-3 bg-red-50 border border-red-200 rounded-xl space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button onClick={() => handleWhatsAppEscalation(entry)} className={`py-2 px-1 text-[10px] md:text-xs font-bold rounded-lg border flex items-center justify-center shadow-sm transition-all ${entry.escalationSent ? 'bg-green-600 text-white border-green-700' : 'bg-white border-red-300 text-red-700 hover:bg-red-100'}`}><MessageSquare size={14} className="mr-1.5" /> {entry.escalationSent ? "Escalated" : "WhatsApp"}</button>
                          {entry.followUpEmail && <button onClick={() => handleSendEmail(entry, 'escalation')} className={`py-2 px-1 text-[10px] md:text-xs font-bold rounded-lg border flex items-center justify-center shadow-sm transition-all ${entry.escalationSent ? 'bg-green-600 text-white border-green-700' : 'bg-white border-red-300 text-red-700 hover:bg-red-100'}`}><Mail size={14} className="mr-1.5" /> Email</button>}
                        </div>
                      </div>
                    )}

                    {!entry.isArchived && entry.guestEmail && (
                      <button onClick={() => handleSendEmail(entry, 'guest')} className={`w-full font-bold py-2.5 rounded-xl border transition-all text-xs md:text-sm flex items-center justify-center shadow-sm active:scale-[0.98] ${entry.guestEmailSent ? 'bg-[#595733]/10 border-[#595733]/30 text-[#595733]' : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-200'}`}>{entry.guestEmailSent ? <CheckCircle size={16} className="mr-2" /> : <Mail size={16} className="mr-2 text-gray-400" />} {entry.guestEmailSent ? 'GUEST NOTIFIED' : 'Email Guest'}</button>
                    )}
                    
                    {entry.status === 'open' && !entry.isArchived && (
                      <>
                        {entry.followUpEmail && !sla.isBreached && (
                          <button onClick={() => handleSendEmail(entry, 'manager')} className={`w-full py-2.5 rounded-xl font-bold text-xs md:text-sm flex items-center justify-center shadow-sm transition-all active:scale-[0.98] ${entry.managerEmailSent ? 'bg-green-600 text-white' : 'bg-[#003040] text-white hover:bg-[#003040]/90'}`}>{entry.managerEmailSent ? <CheckCircle size={16} className="mr-2" /> : <AlertCircle size={16} className="mr-2" />} {entry.managerEmailSent ? (activeSeverity === 'critical' ? 'GM NOTIFIED' : 'HOD ALERTED') : (activeSeverity === 'critical' ? 'Escalate to GM' : 'Alert HOD')}</button>
                        )}
                        {activeSeverity === 'critical' && !sla.isBreached && (
                          <button onClick={() => handleWhatsAppEscalation(entry)} className="w-full bg-[#595733] hover:bg-[#595733]/90 text-white font-bold py-2.5 rounded-xl text-xs md:text-sm flex items-center justify-center shadow-sm active:scale-[0.98] transition-all"><MessageSquare size={16} className="mr-2" /> WhatsApp GM Link</button>
                        )}
